...
namespace Jobspect.Modules.Applications.Persistence;
...
internal sealed class ApplicationsDbContextFactory : IDesignTimeDbContextFactory<ApplicationsDbContext>
{
    ...
    // migration reads a full connection from JOBSPECT_DESIGN_TIME_CONNECTION.
    private const string DesignTimeFallback = "Host=localhost;Port=5432;Database=jobspect;Username=postgres";

    public ApplicationsDbContext CreateDbContext(string[] args)
    {
        var connectionString =
            Environment.GetEnvironmentVariable("JOBSPECT_DESIGN_TIME_CONNECTION") ?? DesignTimeFallback;
