...
- **Status:** Accepted
- **Date:** 2026-07-29
- **Amended:** 2026-07-31 — the timestamp guard is per fact group, not per row, and per assignment rather than per statement; monotone facts need no guard; the funnel needs its own columns; ties apply; the module also holds one row it was told rather than derived; and the figures are served with no cache in front of them. See *Revision history*.
...
Figures are computed from the base rows on every read, with **no cache in front of them**. **Rebuilding means recomputing from the base rows** — not replaying the stream, which is not available.

The absence of a cache is a decision rather than an omission, and it is the same one this record makes everywhere else: at the volume one account reaches, the work is not worth arranging around. A dashboard panel is one owner-scoped scan of a few hundred rows on an indexed column. Caching it would remove single-digit milliseconds from a request a person triggers a handful of times in a sitting, and it would cost more than it sounds — the framework declines to cache responses to authenticated requests, so enabling it here means switching that protection off and hand-rolling per-caller keying in its place. That trades a measurable correctness risk, one account's figures reaching another, for an unmeasurable saving.

The trigger that reverses this is a measured one: sustained p95 latency on the dashboard endpoints, or a second API instance making the same query repeatedly. Neither has fired. Until one does, every read here is a fresh read, which also means the figures are exactly as current as the projections behind them and no panel can disagree with another about how stale it is.
...
- Aggregating on read costs more than incrementing a counter. At the volume one person's job search reaches — hundreds of rows over a year, not millions — a grouped scan is not a cost worth designing against, and it is cheap enough that it needs nothing in front of it. If the shape were ever wrong it would be wrong slowly and visibly, which is the failure mode to prefer.
...
- **Putting an output cache in front of the read endpoints** — rejected; see *Aggregation and rebuild*. It was planned, and the planning is what killed it: the saving is unmeasurable at this volume, while caching authenticated responses means disabling the framework's own refusal to do so and becoming responsible for per-caller keying by hand. Recorded rather than dropped quietly, because the reasoning is what makes the trigger legible if the numbers ever change.
...
- **2026-07-31 — there is no output cache, and this record had assumed one twice.** Two passages described the figures as being served "behind the output cache", which was never built and, on examination, should not be. The saving is unmeasurable against a few hundred owner-scoped rows; the cost is that ASP.NET Core refuses by default to cache responses to authenticated requests, so having one means disabling that protection and owning per-caller keying by hand — a correctness risk taken on for a performance gain nobody had observed. Restated as a decision with a measured trigger rather than an assumption, and a consequence rewritten that had leaned on the cache to justify aggregating on read.
